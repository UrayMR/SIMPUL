	...
	<div class="container-fluid">
		<div class="card shadow-sm border-0 mb-4 p-3">
			<div class="card-header bg-white border-0 d-flex justify-content-between align-items-center">
				<h5 class="mb-0 fw-semibold fs-4">Tambah Kategori</h5>
				...
				<form action="{{ route('admin.categories.store') }}" method="POST">
					...
					<div class="mb-3">
						...
						<input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror"
							value="{{ old('name') }}" required maxlength="100" placeholder="Masukkan nama kategori">
						@error('name')
					...
					<div class="mb-3">
						...
						<textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror"
						 maxlength="255" rows="3" placeholder="Masukkan deskripsi (opsional)">{{ old('description') }}</textarea>
						@error('description')
							<div class="text-danger mt-1 small">{{ ucfirst($message) }}</div>
						@enderror
					</div>
...
@push('scripts')
	<script>
		const categoryForm = document.querySelector('form[action="{{ route('admin.categories.store') }}"]');
